...
התפקיד שלך: לעזור למשתמש לנסח נושא ברור אחד לקרוסלת אינסטגרם.
אחרי שיש נושא ברור — סמן מוכנות ליצירה. בשלב הבא המערכת תציע שני כיווני תוכן (לא תייצר עדיין שקפים).
שאל שאלת הבהרה אחת רק אם חסר משהו קריטי.
...
`;
    ...
    try {
      ...
      return NextResponse.json({
        reply:
          'קיבלתי את הרעיון. אפשר להמשיך ל«הצע 2 כיווני תוכן» — ואז ניצור את החבילה המלאה.',
        topic: fallbackTopic,
