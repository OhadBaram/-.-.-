...
export default function Home() {
  ...
  return (
    <main className="min-h-screen bg-gray-50 dark:bg-gray-900 font-sans transition-colors duration-300" dir="rtl">
      {/* Navbar */}
      <nav className="w-full bg-white dark:bg-gray-900 shadow-sm py-4 px-4 md:px-8 flex justify-between items-center relative z-10 transition-colors duration-300 border-b border-transparent dark:border-gray-800">
        ...
        <div className="flex items-center gap-3 md:gap-6">
          ...
          {session ? (
            ...
            <div className="flex items-center gap-2 md:gap-4">
              <button onClick={() => signIn(undefined, { callbackUrl: '/dashboard' })} className="font-medium text-indigo-600 dark:text-indigo-400 hover:text-indigo-800 dark:hover:text-indigo-300 text-sm md:text-base hidden sm:block">התחברות</button>
              <button onClick={() => signIn(undefined, { callbackUrl: '/dashboard' })} className="px-3 md:px-4 py-2 bg-indigo-600 text-white rounded-lg font-bold text-sm md:text-base hover:bg-indigo-700 transition">
                התחל בחינם
              </button>
            </div>
          )}
      ...
      <section id="features" className="py-20 bg-white dark:bg-gray-950 transition-colors duration-300">
        <div className="max-w-6xl mx-auto px-4">
          <div className="text-center mb-12">
            <div className="inline-block bg-indigo-50 dark:bg-indigo-950/60 text-indigo-700 dark:text-indigo-300 font-bold px-4 py-2 rounded-full text-sm mb-4 border border-transparent dark:border-indigo-800">
          ...
          <div className="grid md:grid-cols-3 gap-8 mb-16">
            <div className="p-8 bg-indigo-50 dark:bg-gray-800 rounded-3xl border border-indigo-100 dark:border-gray-700">
              <div className="w-14 h-14 bg-indigo-600 text-white flex items-center justify-center rounded-2xl mb-5 text-3xl">🌐</div>
              <h3 className="text-xl font-bold text-gray-900 dark:text-gray-50 mb-3">קורא את האתר שלך</h3>
              <p className="text-gray-700 dark:text-gray-300 leading-relaxed">מזין כתובת האתר שלך פעם אחת, וה-AI לומד את המוצרים, הערכים, השפה והייחוד שלך לעומק. כל קרוסלה עתידית תגלם את זה.</p>
            </div>
            <div className="p-8 bg-purple-50 dark:bg-gray-800 rounded-3xl border border-purple-100 dark:border-gray-700">
              <div className="w-14 h-14 bg-purple-600 text-white flex items-center justify-center rounded-2xl mb-5 text-3xl">📸</div>
              <h3 className="text-xl font-bold text-gray-900 dark:text-gray-50 mb-3">לומד מהסגנון שאהבת</h3>
              <p className="text-gray-700 dark:text-gray-300 leading-relaxed">מוסיף קישורים לקרוסלות הטובות שיצרת בעבר, וה-AI ינתח את המבנה, הטון והאסתטיקה – ויחזור אליך עם עוד מאותו הדבר שעבד.</p>
            </div>
            <div className="p-8 bg-blue-50 dark:bg-gray-800 rounded-3xl border border-blue-100 dark:border-gray-700">
              <div className="w-14 h-14 bg-blue-600 text-white flex items-center justify-center rounded-2xl mb-5 text-3xl">⚡</div>
              <h3 className="text-xl font-bold text-gray-900 dark:text-gray-50 mb-3">חיסכון של שעות בשבוע</h3>
              <p className="text-gray-700 dark:text-gray-300 leading-relaxed">מה שלקח שעתיים של עיצוב, קופירייטינג ותיקונים – קורה עכשיו ב-30 שניות. בדיוק בסגנון שלך, בדיוק לקהל שלך, בדיוק בזמן שלך.</p>
            </div>
          </div>

          {/* Social proof strip */}
          <div className="bg-gray-900 dark:bg-black rounded-3xl p-8 text-center text-white border border-transparent dark:border-gray-800">
      ...
      {/* CTA Section */}
      <section className="py-20 bg-indigo-600 dark:bg-indigo-950 text-white text-center transition-colors duration-300 border-t border-transparent dark:border-indigo-900">
        <div className="max-w-4xl mx-auto px-4">
          <h2 className="text-3xl font-bold mb-6">מוכנים לקחת את התוכן שלכם לשלב הבא?</h2>
          <p className="text-indigo-100 dark:text-indigo-200 mb-10 text-lg">
            הצטרפו למאות יוצרי תוכן ובעלי עסקים שכבר משתמשים במערכת שלנו כדי לייצר תוכן ויראלי.
          </p>
          <div className="flex flex-col sm:flex-row justify-center gap-4 items-center">
            <Link href="/pricing" className="inline-block px-10 py-4 bg-white text-indigo-600 rounded-full text-xl font-black hover:bg-gray-100 transition shadow-xl transform hover:-translate-y-1 text-center">
              לצפייה במחירים
            </Link>
            <Link href="/dashboard/create" className="inline-block px-10 py-4 bg-indigo-500 dark:bg-indigo-800 border-2 border-white text-white rounded-full text-xl font-black hover:bg-indigo-400 dark:hover:bg-indigo-700 transition shadow-xl transform hover:-translate-y-1 text-center">
              למעבר ליצירת קרוסלה
            </Link>
          </div>
        </div>
      ...
      {/* Footer */}
      <footer className="py-8 bg-gray-900 dark:bg-black text-gray-400 text-center text-sm border-t border-transparent dark:border-gray-800">
        <div className="mb-4">
